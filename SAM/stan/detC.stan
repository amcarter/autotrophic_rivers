...
parameters {
  ...
  vector<lower=0>[ndays] logC; // organic carbon
}
...
model {
  ...
  //initialize
  Chat[1] = C0;
  logC[1] ~ normal(log(Chat[1]), sigma_proc/10);
  R[1] = AR[1]-K[1]*exp(logC[1]);

  // iterate through timesteps:
  for(i in 2:ndays){
    Chat[i] = (exp(logC[i-1]) + litter[i] + R[i-1] + P[i-1])*(1 - beta_s*Q[i]);
    logC[i] ~ normal(log(Chat[i]), sigma_proc/10);
    R[i] = AR[i] - K[i] * exp(logC[i]);
  }
