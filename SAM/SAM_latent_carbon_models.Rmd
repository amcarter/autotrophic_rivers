...
ss <- auto %>%
    mutate(light = light/max(light),
           ...
           R = 0.5,
           R_obs = ER_C) %>%
    select(date, P = GPP_C, ER = ER_C, R_obs, R, light, Q, temp_C, litter, C)
```
...
pto_mod <- stan('SAM/stan/detC_logpi_1.stan', 
                data = stan_dat, init = 0,
                chains = 4, cores = 4, 
                warmup = 500, iter = 2000)

saveRDS(pto_mod, 'SAM/stan/fits/detC_logpi_1_pto.rds')
```

```{r, echo = FALSE}
pto_mod <- readRDS('SAM/stan/fits/detC_logpi_1_pto.rds')
print(pto_mod, pars = c('beta_s', 'sigma_obs', 'sigma_proc'))
traceplot(pto_mod, ncol = 1, pars=c('beta_s', 'sigma_obs', 'sigma_proc'))
...
plot_post_sim(mod, pars = c('beta_p', 'beta_s', 'sigma_obs', 'sigma_proc'), 
              vals = c( beta_p, beta_s, sigma_obs, sigma_proc))
...
pto_mod <- stan('SAM/stan/P1_detC_logpi.stan', 
                data = stan_dat, init = 0,
                chains = 4, cores = 4, 
                warmup = 500, iter = 2000)
beep(8)
saveRDS(pto_mod, 'SAM/stan/fits/P1_detC_logpi_pto.rds')
```

```{r, echo = FALSE}
pto_mod <- readRDS('SAM/stan/fits/P1_detC_logpi_pto.rds')
print(pto_mod, pars = c('beta_p', 'beta_s', 'sigma_obs', 'sigma_proc'))
traceplot(pto_mod, ncol = 1, pars=c('beta_p', 'beta_s', 'sigma_obs', 'sigma_proc'))
...
## Version: detC_logpi_2  
Contains:  
- Latent detrital carbon
- fixed C0
- log process error
- saturating scour term (segatto et al 2020)

Notes:
This model is on hold because it takes ar eally long time to run, likely because of the iterative selection of the tau0 term? It would be really nice to see Segatto's code and learn how they implemented the shear stress equation
...
ss <- wbp %>%
    mutate(light = PAR_surface/max(PAR_surface),
           ...
           R = 0.5,
           R_obs = ER) %>%
    select(date, P = GPP, R_obs, R, light, Q, temp_C, litter, C, tau_mgm2)
...
for(i in 2:ndays){
  Chat[i] = ss$C[i-1] * (1 - beta_s * ss$ss[i]) + ss$litter[i] + ss$R[i-1] + ss$P[i-1]
  ss$C[i] = exp(rnorm(1, log(Chat[i]), sigma_proc))
  ss$R[i] = ss$AR[i] - ss$K[i]*ss$C[i]
}
...
ss %>% select(date, P, Q, AR, R, C, litter )%>%
pivot_longer( -date, names_to = 'var', values_to = 'value') %>%
  ggplot(aes(date, value, col = var)) +
  geom_line()+
  facet_wrap(.~var, scales = 'free_y', ncol = 2)
...
```{r, comment='', echo = FALSE}
cat(read_lines('SAM/stan/detC_logpi_2.stan'), sep = '\n')
```

```{r, eval = FALSE}
stan_dat <- list(ndays = nrow(ss), R_obs = ss$R_obs, P = ss$P, C0 = 100, 
                 temp = ss$temp_C, litter = ss$litter, tau = ss$tau_mgm2)

mod <- stan('SAM/stan/detC_logpi_2.stan', 
             data = stan_dat, 
             chains = 4,  cores = 4, 
             warmup = 500, iter = 1000)
saveRDS(mod, 'SAM/stan/fits/detC_logpi_2_wbp.rds')
```

### Model Fit

```{r, echo = FALSE}
mod <- readRDS('SAM/stan/fits/detC_logpi_2_wbp.rds')
print(mod, pars = c('beta_s', 'tau0', 'sigma_obs', 'sigma_proc'))
traceplot(mod, ncol = 2, pars = c('beta_s','tau0', 'sigma_obs', 'sigma_proc'))
plot_post_sim(mod, pars = c('beta_s', 'tau0', 'sigma_obs', 'sigma_proc'), 
              vals = c( beta_s, tau0, sigma_obs, sigma_proc))
...
```{r}
ggplot(ss, aes(date, ER_C)) +
    geom_line(col = 'sienna') +
    geom_line(aes(y = GPP_C), col = 'forestgreen')+
    ylab('Metabolism') 

ggplot(ss, aes(date, ER)) +
    geom_line(col = 'sienna') +
    geom_line(aes(y = P), col = 'forestgreen')+
    ylab('Metabolism') + ylim(-10,10)
ggplot(ss, aes(date, litter/4.2), seze = 1.5) +
    geom_line(col = 'sienna') +
    geom_line(aes(y = LAI), col = 'forestgreen')+
    ylab('Terrestrial LAI')

ss %>%
    mutate(logQ = log(Q)) %>%
    select(date, light, logQ, temp_C) %>%
    pivot_longer(-date, names_to = 'variable', values_to = 'value') %>%
...
ss <- wbp %>%
    mutate(light = PAR_surface/max(PAR_surface),
           ...
           R = 0.5,
           R_obs = ER) %>%
    select(date, P = GPP, ER, R_obs,HR,  R,LAI,  light, Q, temp_C, litter, C)
