...
cat(read_lines('src/SAM/stan/SAM_detC_1.stan'), sep = '\n')
```

```{r, eval = FALSE}
ss <- ss[1:30,]
stan_dat <- list(ndays = nrow(ss), nweights = nweight, 
                 R_obs = ss$ER_C, P = ss$GPP_C, 
                 temp = ss$temp_C, tau = ss$tau_mgm2, litter = ss$litter)

mod <- stan('src/SAM/stan/SAM_detC_1.stan', 
             data = stan_dat, 
             chains = 1,  cores = 4, 
             warmup = 200, iter = 500)
saveRDS(mod, 'src/SAM/stan/fits/SAM_detC_1_potomac.rds')
```

un Stan model ####
```{r, echo = FALSE}
mod <- readRDS('src/SAM/stan/fits/SAM_detC_1_potomac.rds')
print(mod, pars = c('C0', 'K_20', 'beta_s', 'tau0', 'beta_p', 'w', 
                    'sigma_obs', 'sigma_proc'))
traceplot(mod, ncol = 1, pars=c('C0', 'K_20', 'beta_s', 'tau0', 'beta_p', 'w', 
                    'sigma_obs', 'sigma_proc'))
plot_post_sim(mod, pars = c('C0', 'K_20', 'beta_s', 'tau0', 'beta_p', 'w', 
                    'sigma_obs', 'sigma_proc'), 
              vals = c(C0, K20, beta_s, tau0, beta_p, w, sigma_obs, sigma_proc))
```
