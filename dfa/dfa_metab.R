...
# load in data ####
library(tidyverse)
dat <- read_csv('C:/Users/Alice Carter/git/autotrophic_rivers/data/autotrophic_siteyears_daily_filled_2.csv')

# look for correlations in the data 
tmp <- dat %>% select(kO2, depth_m, discharge_m3s, DO.obs, light_PAR, 
                      temp.water, DO.amp, iT_norm, slope, Lat, StreamOrde,
                      Width, WS_area_km2, GPP_C_filled, ER_C_filled) %>%
  filter(!is.na(discharge_m3s), !is.na(temp.water), !is.na(kO2))
apply(tmp, 2, function(x) sum(is.na(x)))
corrplot(cov(scale(tmp)))
yy <- dat %>% 
  group_by(sitecode, DOY) %>%
  summarize(GPP = mean(GPP_C_filled, na.rm = TRUE)) %>% 
            # ER = mean(ER_C_filled, na.rm = TRUE)) %>%
  ungroup() %>%
  pivot_wider(names_from = DOY, values_from = GPP) %>%
  select(-sitecode, -'366')
corrplot(cov(scale(yy[,-1])))

# xx <- dat %>%
#   group_by(sitecode, DOY) %>%
#   summarize(across(c('discharge_m3s', 'light_PAR', 'temp_water')),
#             mean, na.rm = TRUE) %>%
#   ungroup() %>%
#   pivot_wider(names_from 30 # number of time series
#   
...
plot(seq(1,TT), xx[1,], type = 'b', ylim = c(-3,3), xlab = 'time', ylab = 'x_t',
     main = 'cumulative proc error')
lines(seq(1,TT), xx[2,], type = 'b', col = 'goldenrod')
lines(seq(1,TT), xx[3,], type = 'b', col = 'purple')
...
plot(1:TT, yy[1,], type = 'l', ylim = c(0,10))
for(i in 2:NN){ lines(1:TT, yy[i,])}
...
## corrrelation
cor_yy <- matrix(diag(cor(t(yy_z), t(yy_fit))), NN/6, 6)
## plots
