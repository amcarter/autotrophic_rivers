...
# log model centered X_t
sink('src/SAM/stan/ar1_model_logR_cent.stan')
cat("
  data {
    int <lower = 0> N;  // Sample size
    vector[N] logR_obs; // log ER observed
  ...
  model {
    logR[1] ~ normal(logR_obs[1], sigma_obs);
    logR[2:N] ~ normal(b0*(1-phi) + phi * logR[1:(N-1)], sigma_proc);
    logR_obs ~ normal(logR, sigma_obs);
  ...
  generated quantities {
    ...
    R_tilde[1] = logR[1];
    for(i in 2:N){
      R_hat[i] = normal_rng(b0 * (1-phi) + phi * R_hat[i-1], sigma_proc);
      R_tilde[i] = normal_rng(R_hat[i], sigma_obs);  
    }
  }", fill = T)
...
logR_obs = rnorm(1, logR[1], sigma_obs)
for (i in 2:100){
  logR[i] <- b0*(1-phi) + phi * logR[i-1] + rnorm(1, 0, sigma_proc)
  logR_obs[i] = rnorm(1, logR[i], sigma_obs)
}
...
sim_dat <- list(logR_obs = logR_obs, N = length(logR_obs), mu_obs = sigma_obs)
fit <- stan(file = 'src/SAM/stan/ar1_model_logR_cent.stan', data = sim_dat,
